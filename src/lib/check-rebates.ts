...
//
// Phần việc nằm ở ĐÂY chứ không nằm trong `app/api/check-rebates/route.ts`, để
// chạy được từ cả hai phía: route (webhook, gọi tay) và
// `scripts/check-rebates.mts` (runner của GitHub Actions). Lý do tách ra là
// chuyện đo được ngày 09/09/2026 — xem đầu file script.
...
export interface CheckError {
  ...
  message: string;
  /**
   * `true` khi lỗi có thể tự khỏi ở lượt chạy lại — chỉ đúng với lỗi mạng khi
   * đọc một trang FinlyWealth. Mọi lỗi còn lại (slug va chạm, chữ lệch số,
   * entry có bản nháp) là trạng thái của DỮ LIỆU: chạy lại mười lượt vẫn ra
   * đúng câu đó, và mỗi lượt là mười lần đọc FinlyWealth đổ đi.
   *
   * Trước 09/09/2026 không có phân biệt này: workflow gọi bằng
   * `curl --retry`, mà curl chỉ nhìn thấy HTTP 500 nên nó chạy lại CẢ hai
   * loại. Xem `scripts/check-rebates.mts`.
   */
  retryable?: boolean;
}

export interface CheckRebatesResult {
  checked: number;
  updated: Change[];
  unchanged: string[];
  errors: CheckError[];
...
export interface CheckRebatesConfig {
  spaceId: string;
  managementToken: string;
}

/**
 * Gọi lại an toàn: mọi nhánh ghi đều đối chiếu với bản ĐÃ PUBLISH trước, nên
 * chạy hai lần liền không tạo bản trùng và không publish thừa.
 */
export async function runCheckRebates({
  spaceId,
  managementToken,
}: CheckRebatesConfig): Promise<CheckRebatesResult> {
  ...
  // mỗi ngày, nên nó thấy; và vì điều kiện còn nguyên ở mọi lượt sau, vòng thử
  // lại của workflow không rửa nó thành xanh.
  ...
  // mỗi ngày. Cùng lý do đã đặt lượt canh slug ngay bên trên vào đây.
  ...
  // lỗi còn nguyên ở mọi lượt sau, vòng thử lại không rửa thành xanh.
  for (const [slug, liveCard] of published) {
    ...
    // trả đồng nào. Đó là tiền hứa với người đọc, đúng thứ cả job này sinh ra
    // để canh.
    //
    ...
    // Điều kiện này đọc từ bản ĐÃ PUBLISH nên còn nguyên ở mọi lượt sau, vòng
    // thử lại không rửa được thành xanh.
  ...
  return { checked, updated, unchanged, errors };
}
