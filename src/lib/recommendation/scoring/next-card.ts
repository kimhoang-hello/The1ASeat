...
export function benefitsFit(candidate: CandidateFacts, ctx: ScoringContext): { raw: number; note: string } {
  ...
  return {
    raw: 0.6 * cash + 0.4 * count,
    note:
      `quyền lợi TĂNG THÊM (§16 Rule 6): 0.6 × tiền ${cash.toFixed(2)} + 0.4 × số lượng ${count.toFixed(2)}` +
      ` (${candidate.benefits.incrementalCount} quyền lợi mới)`,
  };
}
...
export function scoreNextCard(
  ...
  const share = existingShare(candidate, ctx);
  const diversificationRaw = (1 - share) * (0.5 + 0.5 * ctx.needs.portfolio.diversification);
  const benefits = benefitsFit(candidate, ctx);
  ...
  return [
    ...
    component(
      ...
      0.1,
      diversificationRaw,
      `mở ra chỗ mới (1 − tỷ trọng đã có ${share.toFixed(2)}) × nhu cầu đa dạng hoá ` +
        `(0.5 + 0.5 × ${ctx.needs.portfolio.diversification.toFixed(2)})`,
    ),
  ];
}
